...
- **Slowly changing dimensions**(Dimension data does not change very often, but it is used alongside fact data that changes regularly.)
...
- **Power BI**(analytics suite - for reporting)
- **Qlik**((Analytics suite). SaaS platform)
...
- **Minitab**(statistics package. Shares most of the same features like SPSS,SAS,STATA not widely used  today)
