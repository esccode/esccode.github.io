...
- **Integration**
  - *Extract, transform, load*(ETL is the most commonly used method for *data warehouses*. `ETL` transformation occour outside the database.Combines data from multiple data sources into a single, `consistent data store that is loaded into a data warehouse` or other target system.)
  ...
  - *Application programming Interfaces(APIs)*(is a set of protocols within a computer system that `allows two unrelated systems to communicate`. APIs have been around for a long time, but they have gained more visibility in the data world in the last decade. Defined simply, an APIis the ability to share data across systems.)
- **Data collection methods**
  ...
  - *Interface(API)/web services*( The key difference between a web service and most other APIs is the use of a hosted network. Web services require this network and use different protocols to transfer the data between machines. Web services use XML because this markup language is generic in nature, allowing the data to be transferred to a multitude of systems that are designed in many different languages. Web services are typically dedicated and designed to a particular process)
  - *Survey*(A survey is defined as the act of examining a process or questioning a selected sample of individuals to obtain data about a service, product, or process. `Data collection surveys` collect information from a targeted group of people about their opinions, behavior, or knowledge. Allows us to gather data about people and their habits. `Multiple-choice questions` allow the survey taker to choose from predefined options. `Open-ended questions` allow free-form text responses)
...
- **Data outliers**(An outlier is a data point an abnormal distance from the other values. But there is not an exact definition of abnormal. To identify outliers is easy to use visualization tool **`Boxplot`**. There is no standard definition to identify an outlie)
...
- **Data merge**(`Adds columns`,the process of combining two or more datasets into a single dataset)
...
- **Data append**(`Adds rows`, a process that involves adding new data elements to an existing database. An `inline append` will combine data sets until you achieve your final result; all the sets are combined, leaving just the combined set.
...
- **Parsing**(Means to break data into parts)/**string manipulation**((or string handling) is the `process of changing, parsing, splicing pasting, or analyzing strings`. SQL is used for managing data in a relational database, e.g `SELECT address, REPLACE(address, ‘St’, ‘Street’) AS new_address FROM melb LIMIT 5;`)

### 2.4 Explain common techniques for data manipulation and query optimization

- **Data manipulation**
  - *Filtering*(Filters allow you to `show or hide information` on your sheet based on selected criteria.`SELECT ORDERID, ORDERDATE, ORD_AMOUNT FROM Orders WHERE ORDERDATE > 1/1/2020`)
...
- **Query optimization**
  ...
  - *Indexing*(Is a `field property setting` that tells the database that a field needs to be indexed. When fields are indexed, it makes data `processing faster and ultimately speeds up the performance` of the query but also `slowdown of inserting information into the database`. Indexing makes `columns faster to query` `by creating pointers to where data is stored` within a database.)
...
- **Measures of central tendency**
  ...
  - *Median*(The `middle` number within a group of sorted numbers. In Exel `=MEDIAN(Table2[Column1])`. When the distribution has an `even` number of observations, the 
...
- **t-tests**(A test used to `compare two groups` when determining whether there is a `significant difference` between the `means of both groups`. Is used to produce a `p value` when you conducting hypothesis testing. A t-test is used as a `hypothesis testing tool`, which allows testing of an assumption applicable to a population. There are two important variables when conducting a t-test. The first, which is the dependent variable, is what is being measured`. The second variable associated with t-tests is the independent variable. An `independent variable is the difference between groups`.)
- **Z-score**(`Subtract` the `mean` from the `value` and `divide` this quantity by the `standard deviation`. A value that **shows how many standard deviations a data point is from the mean**.In Excel `STANDARDIZE (x,mean,standard_dev)`. A Z-score is a numerical measurement that describes a value’s relationship to the mean of a group of values. Z-score is measured in terms of standard deviations from the mean `z = (X – μ) / σ`. E.g. `A certain data set has a standard deviation of 20 and a mean of 150. One of the values in the data set is 120, what is the Z-score for this data point?`Answer is `The Z-score is -1.5. (120-150) / 20.`)
...
- **Process to determine type of analysis**
  ...
  - *Scoping/gap analysis*(Gap analysis studies the present state, desired state, and the gaps between the two.)
...
- **IBM SPSS Modeler**(SPSS Modeler has been used in these and other industries:
...
select a month to include in the report or `to choose specific start and end dates` for the data included in the report.)
- **Frequency**
...
- **Data quality rule and metrics**
  - *Conformity*(Conformity in data is a data quality dimension that measures how well the data follows the standard data definitions like data type, size, and format.)
  - *Non-conformity*(Non-conforming data refers to data that does not meet certain requirements or expectations. There are different types of non-conforming data, such as data elements whose values are missing, incomplete, or not consistent with other values in the same or comparable reports)
  - *Rows passed*(your job role may involve letting your managers know how many rows passed through all the quality checks)
  - *Rows failed*(your job role may involve letting your managers know how many rows failed to meet the standard)
- **Methods to validate quality**
  ...
  - *Data audits*(The process of assessing that the data achieves a specific objective or purpose)
