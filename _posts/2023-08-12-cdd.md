...
- **Hypothesis testing**(An `alternative hypothesis(Ha)` The statment that we are trying to prove is correct, while a `null hypothesis(H0)` there has no been change or no difference.)
