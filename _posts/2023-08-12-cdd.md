...
- **P-values**(Probability that test results are not statistically significant. A `high p-value` indicates that the outcome is likely not repeatable and thus not significant, whereas a `low p-value` tells us the event is likely repeatable, meaning itss did not occur due to chance and is thus statistically significant.)
