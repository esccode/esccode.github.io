...
- **Static vs. dynamic reports**(`Static reporting` requires pulling up various reports from different sources and analyzing insights from a longer time period in order to provide a snapshot of data while `Dynamic reports` provide deep insights, and allow users to interact with the data rather than just view it.)
